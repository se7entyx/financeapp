<x-layout>
    ...
    <section class="bg-white dark:bg-gray-900 w-full">
        <div class="py-4 px-8 mx-auto max-w-7xl">
            <form id="my-form" action="/dashboard/edit/bukti-pengeluaran/{{$buktiKasRecords->id}}" method="post">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-span-4">
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" id="buktiTable">
                                ...
                                <tbody>
                                <tbody>
                                    @foreach($buktiKasRecords->keterangan_bukti_kas as $index => $item)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <td class="px-6 py-4">{{ $index + 1 }}</td>
                                        <td class="px-6 py-4">{{ $item->keterangan }}</td>
                                        <td class="px-6 py-4">{{ $item->dk }}</td>
                                        <td class="px-6 py-4">{{ $item->jumlah }}</td>
                                        <td class="px-6 py-4">
                                            <button type="button" class="edit-btn text-blue-600 hover:underline" data-id="{{ $item->id }}">Edit</button>
                                            <button type="button" class="delete-btn text-red-600 hover:underline" data-id="{{ $item->id }}">Delete</button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
